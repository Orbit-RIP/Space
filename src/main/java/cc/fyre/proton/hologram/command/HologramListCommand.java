...
public class HologramListCommand {
    ...
    public static void execute(CommandSender sender) {
        ...
        Proton.getInstance().getHologramHandler().getCache().forEach((key,value) -> new FancyMessage(ChatColor.GRAY + "-> " + ChatColor.RED + key + "").command("/tppos " + value.getLocation().getX() + " " + value.getLocation().getY() + " " + value.getLocation().getZ()).send(sender));
